...
imgFolder = '../../reports/generatedImgs/'
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if (iters % 500 == 0) or ((epoch == num_epochs-1) and (i == len(dataloader)-1)):
                ...
                img_name = 'fakeCity_' + str(epoch) + '_' + str(i) +'.png'
                save_image(img_list[0], imgFolder+img_name)

            iters += 1
        # generate image from same noise
        fake = netG(fixed_noise).detach().cpu()
...
def suggest_hyperparameters(trial):
    ...
    return lr, weight_decay, convsG, convsD, drop_conv2
...
def objective(trial: optuna.Trial):
    ...
    with mlflow.start_run():
        ...
        if mse_errG <= best_mse_val:
            ...
            # save one image per epoch to evaluate the progress
            # for i in range(n_epochs):
            #     if i%25==0:
            #         img_name = 'fakeCity_' + str(i) +'.png'
            #         save_image(img_list[i], imgFolder+img_name)
...
logging.debug(study.best_trial.value)  # Show the best value.
