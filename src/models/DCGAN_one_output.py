...
class Discriminator(nn.Module):
    def __init__(self, nc, ndf):
        ...
        self.main = nn.Sequential(
            nn.Conv2d(in_channels=nc, out_channels=ndf, kernel_size=5, stride=3, padding=1, 
                      bias=False),
            nn.BatchNorm2d(ndf),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_channels=ndf, out_channels=ndf * 2, kernel_size=5, stride=3, padding=1, 
                      bias=False),
            nn.BatchNorm2d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_channels=ndf * 2, out_channels=ndf * 4, kernel_size=5, stride=3, padding=1, 
                      bias=False),
