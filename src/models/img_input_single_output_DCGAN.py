...
class Generator(nn.Module):
    def __init__(self, nz, nc, ngf):
        ...
        self.main = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=ngf, kernel_size=5, stride=1, 
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_channels=ngf * 2, out_channels=ngf * 4, kernel_size=5, stride=2, 
                               padding=0, bias=False),
            nn.BatchNorm2d(ngf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose2d(in_channels=ngf * 4, out_channels=ngf * 2, kernel_size=7, stride=2, 
                               padding=0, bias=False),
            nn.BatchNorm2d(ngf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose2d(in_channels=ngf * 2, out_channels=ngf, kernel_size=6, stride=1, 
                               padding=0, bias=False),
            nn.BatchNorm2d(ngf),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose2d(in_channels=ngf, out_channels=nc, kernel_size=3, stride=1, 
                               padding=0, bias=False)
        )
    ...
    def forward(self, input_):
        ...
        return torch.cat((input_, output), dim=1)
