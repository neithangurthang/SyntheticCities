...
MAX_FRAC = 0.1  # threshold of maximum fraction of buildings
MIN_FRAC = 0.01  # threshold of minimum fraction of buildings
...
for epoch in range(NUM_EPOCHS):
    for i, data in enumerate(dataloader):
        ...
        real_batch = real_batch[:, 0, :, :]  # extracting only roads!
        ...
        errG = - output.mean()  # + COND_RATE * cond_reg
        errG.backward()
        optimizerG.step()
    ...
    if epoch % 5 == 0:
        with torch.no_grad():
            roads = netG(fixed_noise).detach().cpu()
            greens = torch.ones_like(roads) - roads 
            buildings = torch.zeros_like(roads)
